...
setupMock({
  setup: () => {
    Mock.mock(new RegExp('/api/index/overview-content'), () => {
      const year = new Date().getFullYear();
      const getLineData = (type: string) => {
        return new Array(12).fill(0).map((_item, index) => ({
          date: `${year}-${index + 1}`,
          count: Mock.Random.natural(0, 57),
          name: type
        }));
      };
      const legend = ['sent', 'received'];
      return {
        likesYouHave: '98',
        likesYouSent: '23',
        likesYouReceived: '35',
        growthRate: '2.8%',
        chartData: legend.map(getLineData).flat()
      };
    });

    Mock.mock(new RegExp('/api/index/people'), () => {
      return Array(100)
        .fill(0)
        .map((_) => Mock.Random.name());
    });
    Mock.mock(new RegExp('/api/index/polar'), () => {
      const items = categories;
      const category = ['you', 'average'];
